...
void init_logging() {
  ...
  logging::add_file_log(keywords::file_name = "worker.log",
...
public:
  ...
  /// @param propertyName
  template <typename T>
  static void print(std::vector<std::vector<T>> property,
  ...
  template <typename T> static void print(T *array, size_t size) {
    cout << "[";
  ...
  flatten(const std::vector<std::vector<T>> &input) {
    size_t totalSize = 0;
    for (const auto &innerVector : input) {
      totalSize += innerVector.size();
    }
    std::unique_ptr<std::vector<T>> flattened =
        std::make_unique<std::vector<T>>();
    flattened->reserve(totalSize);
    for (const auto &innerVector : input) {
      flattened->insert(flattened->end(), innerVector.begin(),
                        innerVector.end());
    }
    return std::move(flattened);
  }
  //	template <typename T>
  //	static std::unique_ptr<std::vector<std::vector<T>>> expand(MyArray<T>
  //*input, const size_t &cols)
  //	{
  //		std::unique_ptr<std::vector<std::vector<T>>> expanded =
  // std::make_unique<std::vector<std::vector<T>>>(); 		size_t size =
  // input->size()
  /// cols; 		expanded->reserve(size); 		for (size_t i =
  /// 0; i < size; i++)
  //		{
  //			std::vector<T> row(input->firstItem() + i * cols,
  // input->firstItem() + (i + 1) * cols);
...
private:
  ConnectionOptions _build_options() {
    ...
    connection_options.host = std::string(std::getenv("REDIS_HOST"));
    connection_options.port = std::stoi(std::getenv("REDIS_PORT"));
    cout << "Redis Host: " << connection_options.host << endl;
    cout << "Redis Port: " << connection_options.port << endl;
  ...
  static Amqp amqp() {
    ...
    auto channel = AmqpClient::Channel::Create(
        amqp_connector.AMQP_HOST, stoi(amqp_connector.AMQP_PORT),
        amqp_connector.AMQP_USER, amqp_connector.AMQP_PWD,
    ...
    auto consumer_tag = channel->BasicConsume(amqp_connector.AMQP_QUEUE, "",
                                              true, false, false, 1);
  ...
  struct BaseMesh {
    ...
    /// @param mesh_cols
    template <typename T>
    void set_properties(const json &property, const string &property_name,
                        std::vector<std::vector<T>> &mesh_property,
  ...
  // NewMesh is now defined in NewMesh.h
  ...
  /// @param payload
  /// @return ParseResult
  ParseResult json_to_mesh(BaseMesh &mesh, const string *task_id,
                           const json *payload) {
      ...
      if (mesh_json_mapping.find(propertyName) != mesh_json_mapping.end()) {
        ...
        try {
          if (memberName == "coordinates")
            mesh.set_properties(propertyValue, memberName, mesh.coordinates,
                                &mesh.num_nodes, &mesh.num_dims);
          else if (memberName == "elements")
            mesh.set_properties(propertyValue, memberName, mesh.elements,
                                &mesh.num_elements, &mesh.nodes_per_element);
          ...
          BOOST_LOG_TRIVIAL(error)
              << "Error parsing property: " << propertyName
              << " from data payload: " << *task_id << endl;
          break;
        } catch (const json::type_error &e) {
          valid_json = false;
          message_stream << "Unexpected type for: " << propertyName;
          BOOST_LOG_TRIVIAL(error)
              << "Unexpected type for: " << propertyName
              << " from data payload: " << *task_id << endl;
          break;
        }
        continue;
        ...
        BOOST_LOG_TRIVIAL(error) << "Unknown property: " << propertyName
                                 << " from data payload: " << *task_id << endl;
        break;
      }
    }
  ...
  static std::unique_ptr<NewMesh>
  mesh_generation_error(const char *error_message) {
  ...
  static std::unique_ptr<NewMesh> generate_mesh(const BaseMesh &mesh) {
    ...
    std::unique_ptr<std::vector<int>> flattened_elements =
        VectorHelpers::flatten(mesh.elements);
    int *elements = flattened_elements->data();
    std::unique_ptr<std::vector<double>> flattened_coordinates =
        VectorHelpers::flatten(mesh.coordinates);
    double *coordinates = flattened_coordinates->data();
    try {
      for (int e = 0; e < mesh.num_elements; e++)
        fem.generateElement(
            mesh_type, elements + e * (mesh.nodes_per_element),
            mesh.nodes_per_element); //-1 because of extra element
    } catch (const MosException &e) {
      BOOST_LOG_TRIVIAL(error)
          << "Error generating mesh elements: " << e.what();
      return std::move(mesh_generation_error(e.what()));
    }
    try {
      fem.x.setDim(static_cast<int>(mesh.num_dims) *
                   mesh.num_nodes); // casting is not a problem since num_dims
    ...
    try {
      ...
      stringstream error;
      error << "Error splitting Nodes " << node1 << "and " << node2 << " "
            << e.what();
      // return std::move(mesh_generation_error(("Error splitting Nodes: " +
      // node1 + string(", and ") + node2 + "\n" + e.what()).c_str()));
      return std::move(mesh_generation_error(error.str().c_str()));
    }
    std::unique_ptr<NewMesh> new_mesh_results =
        std::move(fem.writeMeshToVector());
    // print out output
  ...
  TaskResponse process_task(const string *task_id, const json *payload) {
    ...
    if (parsed.is_valid) {
      ...
      response.status =
          new_mesh->success ? TaskStatus[SUCCESS] : TaskStatus[FAILED];
      if (new_mesh->success) {
        json data = {{"elements", *new_mesh->elements},
  ...
  void send_results(const string *task_id, json *message) {
    BOOST_LOG_TRIVIAL(info)
        << "Sending results for task: " << *task_id << " to redis" << std::endl;
    try {
      this->redis.set(*task_id, message->dump());
      BOOST_LOG_TRIVIAL(info) << "Results for task: " << *task_id
...
public:
  Worker() : opts_(_build_options()), redis(opts_) { init_logging(); }
  ...
  void consume() {
    ...
    while (keep_running) {
      ...
      auto message_delivered = amqp_members.channel->BasicConsumeMessage(
          amqp_members.consumer_tag, envelope, 1000);
      ...
      json task_payload = {{"task_id", *results.task_id},
                           {"status", results.status},
                           {"payload", results.payload != nullptr
                                           ? *results.payload
                                           : json(nullptr)},
                           {"message", results.message}};
      results.status == TaskStatus[SUCCESS] ? ack_message(envelope)
                                            : reject_message(envelope);
